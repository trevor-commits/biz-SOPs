...
## Purpose
This file is the repo-local Linear/Core routing contract for `SOPs`. The full Trevor-standard workflow lives in `/Users/gillettes/Coding Projects/Linear/LINEAR.md`; repo docs remain authoritative for truth.

## Local Mode
- Live Linear team and prefix: `TODO: verify`
- If this repo is intentionally `repo-only`, say so explicitly here and keep `todo.md` `Linear Issue Ledger` present with a note instead of omitting the section.
- Until the live team and prefix are verified, do not invent issue IDs or automation settings.
