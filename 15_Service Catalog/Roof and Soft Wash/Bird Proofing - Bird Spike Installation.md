---
type: service-description
status: draft
owner: Trevor
created: 2026-04-01
last_reviewed:
next_review:
service_line: Roof and Soft Wash
service_type: add-on
customer_visibility: customer-facing
review_status: needs-review
title_collision: false
source_uuid: olit_95f45b61521f40ce8ca3db8c79bf9b97
price: 0
unit_of_measure:
cost: 0
taxable: false
online_booking_enabled: false
aliases: []
tags:
  - service-description
---

# Bird Spike Installation

> [!summary] Quick Summary
> Once the area is cleaned, we’ll install deterrents such as bird spikes in the problem zones — ledges, rooflines, and vents — to keep birds from nesting or roosting again.

> [!info] Pricing
> - CRM price: $0.00
> - Unit: Quoted / not specified
> - Taxable: No
> - Online booking enabled: No
> - Pricing note: CRM stores this as $0.00. Treat as quoted, complimentary, or incomplete until verified.

## Included
- Inspect the property to find common roosting/nesting spots
- Install bird spikes or other deterrents where needed
- Secure materials without damaging roofing, gutters, or siding
- Test placement to ensure coverage in problem areas

## Not Included
- Full exclusion netting (requires specialized service)
- Pest control for other animals (rodents, insects, etc.)

## Best Fit / When To Offer
- Stops birds from landing and nesting on your property
- Protects against droppings, feathers, and nesting messes returning
- Helps protect your family’s health by reducing exposure to bird waste
- Long-term solution with minimal upkeep

Pricing is based on linear footage, access, and type of deterrent installed. We’ll recommend the best approach for your property after inspection.

## Upsells and Related Services
- [[15_Service Catalog/Roof and Soft Wash/Bird Proofing - Metal Bird Spike Installation|Metal Bird Spike Installation]]
- [[15_Service Catalog/Roof and Soft Wash/Bird Proofing - Plastic Bird Spike Installation|Plastic Bird Spike Installation]]
- [[15_Service Catalog/Roof and Soft Wash/Bird Proofing - Roof Bird Waste Cleaning and Removal|Roof Bird Waste Cleaning and Removal]]

## Internal Notes
- CRM category: Bird Proofing.
- Pricing is not ready for public use without verification.
- This row is not currently marked as online-bookable in the CRM.
- Use the Local Notes and Links section for approved SOP, checklist, equipment, and purchase-note links.

## Local Notes and Links
- Add approved SOP, checklist, equipment, and purchase-note links here.

> [!note]- Source Copy
> Once the area is cleaned, we’ll install deterrents such as bird spikes in the problem zones — ledges, rooflines, and vents — to keep birds from nesting or roosting again.
>
> What we do:
> • Inspect the property to find common roosting/nesting spots
> • Install bird spikes or other deterrents where needed
> • Secure materials without damaging roofing, gutters, or siding
> • Test placement to ensure coverage in problem areas
>
> Why it’s worth it:
> • Stops birds from landing and nesting on your property
> • Protects against droppings, feathers, and nesting messes returning
> • Helps protect your family’s health by reducing exposure to bird waste
> • Long-term solution with minimal upkeep
>
> What’s not included:
> • Full exclusion netting (requires specialized service)
> • Pest control for other animals (rodents, insects, etc.)
>
> Just so you know:
> Pricing is based on linear footage, access, and type of deterrent installed. We’ll recommend the best approach for your property after inspection.

> [!note]- Source Metadata
> - Industry: Window & Exterior Cleaning
> - CRM category: Bird Proofing
> - Source UUID: olit_95f45b61521f40ce8ca3db8c79bf9b97
> - Industry UUID: ind_50413bdcbb67410da504845ec48af17d
> - Source file: 50_Reference/CRM Service Descriptions/GilletteWindowSolarCleaning_pricebook_export.csv

> [!warning]- Audit Notes
> - CRM price is $0.00. Treat this as quoted, complimentary, or incomplete until pricing is verified.

> [!abstract]- Review History
> - 2026-04-01 - Imported from the CRM pricebook export with `scripts/build_service_catalog.py`.
