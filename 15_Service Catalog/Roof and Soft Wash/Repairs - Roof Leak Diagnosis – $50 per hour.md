---
type: service-description
status: draft
owner: Trevor
created: 2026-04-01
last_reviewed:
next_review:
service_line: Roof and Soft Wash
service_type: repair
customer_visibility: customer-facing
review_status: clean
title_collision: false
source_uuid: olit_ba8eab67eadc4305b816488fdbb287a6
price: 50
unit_of_measure:
cost: 0
taxable: false
online_booking_enabled: true
aliases: []
tags:
  - service-description
---

# Roof Leak Diagnosis – $50/hour

> [!summary] Quick Summary
> We’ll track down the source of a roof leak with a focused inspection and targeted water testing so you know what’s going on and how to fix it.

> [!info] Pricing
> - CRM price: $50.00
> - Unit: Quoted / not specified
> - Taxable: No
> - Online booking enabled: Yes
> - Pricing note: Fixed CRM price is present; confirm the pricing basis before using it externally.

## Included
- Inspect roof surfaces, tiles/shingles, flashing, penetrations, and valleys
- Check inside (attic/ceiling areas) if safely accessible
- Controlled hose testing to replicate the leak when appropriate
- Document findings with notes/photos and recommend repairs

## Not Included
- Permanent repairs or materials
- Interior remediation (drywall/paint/flooring)
- Opening walls/ceilings or any destructive testing

## Best Fit / When To Offer
- Pinpoints the actual entry point (not just the stain you see inside)
- Helps you avoid repeat leaks and guesswork repairs
- Gives you a clear plan and estimate for the fix

## Just So You Know
Rate is $50/hour with a 1-hour minimum. Weather and roof safety may limit testing on the day of service. If water testing is used, minor dripping can occur by design to confirm the source.

## Upsells and Related Services
- [[15_Service Catalog/Roof and Soft Wash/Repairs - Roof Tile Repair - Replacement|Roof Tile Repair/Replacement]]
- [[15_Service Catalog/Roof and Soft Wash/Repairs - Seal Leaky Roof Vent|Seal Leaky Roof Vent]]
- [[15_Service Catalog/Roof and Soft Wash/Bird Proofing - Bird Spike Installation|Bird Spike Installation]]

## Internal Notes
- CRM category: Repairs.
- This row is currently marked as online-bookable in the CRM.
- Use the Local Notes and Links section for approved SOP, checklist, equipment, and purchase-note links.

## Local Notes and Links
- Add approved SOP, checklist, equipment, and purchase-note links here.

> [!note]- Source Copy
> We’ll track down the source of a roof leak with a focused inspection and targeted water testing so you know what’s going on and how to fix it.
>
> What we do:
> • Inspect roof surfaces, tiles/shingles, flashing, penetrations, and valleys
> • Check inside (attic/ceiling areas) if safely accessible
> • Controlled hose testing to replicate the leak when appropriate
> • Document findings with notes/photos and recommend repairs
>
> Why it’s worth it:
> • Pinpoints the actual entry point (not just the stain you see inside)
> • Helps you avoid repeat leaks and guesswork repairs
> • Gives you a clear plan and estimate for the fix
>
> What’s not included:
> • Permanent repairs or materials
> • Interior remediation (drywall/paint/flooring)
> • Opening walls/ceilings or any destructive testing
>
> Just so you know:
> Rate is $50/hour with a 1-hour minimum. Weather and roof safety may limit testing on the day of service. If water testing is used, minor dripping can occur by design to confirm the source.

> [!note]- Source Metadata
> - Industry: Window & Exterior Cleaning
> - CRM category: Repairs
> - Source UUID: olit_ba8eab67eadc4305b816488fdbb287a6
> - Industry UUID: ind_50413bdcbb67410da504845ec48af17d
> - Source file: 50_Reference/CRM Service Descriptions/GilletteWindowSolarCleaning_pricebook_export.csv

> [!warning]- Audit Notes
> - No auto-detected issues during import.

> [!abstract]- Review History
> - 2026-04-01 - Imported from the CRM pricebook export with `scripts/build_service_catalog.py`.
