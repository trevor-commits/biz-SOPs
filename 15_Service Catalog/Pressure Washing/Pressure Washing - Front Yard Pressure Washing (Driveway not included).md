---
type: service-description
status: draft
owner: Trevor
created: 2026-04-01
last_reviewed:
next_review:
service_line: Pressure Washing
service_type: core-service
customer_visibility: customer-facing
review_status: needs-review
title_collision: false
source_uuid: olit_0ea934facead4dd9a1ee33e35adba77e
price: 150
unit_of_measure:
cost: 0
taxable: false
online_booking_enabled: true
aliases: []
tags:
  - service-description
---

# Front Yard Pressure Washing (Driveway not included)

> [!summary] Quick Summary
> We’ll pressure wash the main ground-level surfaces in your front yard to remove dirt, moss, algae, and grime. This is a quick but noticeable way to freshen up your entry area and walkways.

> [!info] Pricing
> - CRM price: $150.00
> - Unit: Quoted / not specified
> - Taxable: No
> - Online booking enabled: Yes
> - Pricing note: Fixed CRM price is present; confirm the pricing basis before using it externally.

## Included
- Pressure wash walkways, porch steps, and other concrete or paved areas
- Apply a cleaning solution to help break down organic buildup
- Rinse thoroughly to remove dirt, moss, and surface stains
- Move lightweight items as needed — if anything is fragile or valuable, we ask that it be moved ahead of time

## Not Included
- Driveway cleaning (can be added separately)
- Oil stain removal — some improvement is likely, but stains may not fully come out
- Cleaning of siding, garage doors, or vertical surfaces

## Best Fit / When To Offer
- Instantly improves curb appeal
- Removes slippery moss and buildup from entryways
- Helps preserve your concrete and pavers by clearing off grime

## Just So You Know
Pricing depends on the total area being cleaned and how dirty the surface is.

## Upsells and Related Services
- [[15_Service Catalog/Pressure Washing/Pressure Washing - Backyard Pressure Washing|Backyard Pressure Washing]]
- [[15_Service Catalog/Pressure Washing/Pressure Washing - Concrete Stain Removal|Concrete Stain Removal]]
- [[15_Service Catalog/Pressure Washing/Pressure Washing - Driveway Cleaning|Driveway Cleaning]]

## Internal Notes
- CRM category: Pressure Washing.
- This row is currently marked as online-bookable in the CRM.
- Use the Local Notes and Links section for approved SOP, checklist, equipment, and purchase-note links.

## Local Notes and Links
- Add approved SOP, checklist, equipment, and purchase-note links here.

> [!note]- Source Copy
> We’ll pressure wash the main ground-level surfaces in your front yard to remove dirt, moss, algae, and grime. This is a quick but noticeable way to freshen up your entry area and walkways.
>
> What we do:
> • Pressure wash walkways, porch steps, and other concrete or paved areas
> • Apply a cleaning solution to help break down organic buildup
> • Rinse thoroughly to remove dirt, moss, and surface stains
> • Move lightweight items as needed — if anything is fragile or valuable, we ask that it be moved ahead of time
>
> Why it’s worth it:
> • Instantly improves curb appeal
> • Removes slippery moss and buildup from entryways
> • Helps preserve your concrete and pavers by clearing off grime
>
> What’s not included:
> • Driveway cleaning (can be added separately)
> • Oil stain removal — some improvement is likely, but stains may not fully come out
> • Cleaning of siding, garage doors, or vertical surfaces
>
> Just so you know:
> Pricing depends on the total area being cleaned and how dirty the surface is.

> [!note]- Source Metadata
> - Industry: Window & Exterior Cleaning
> - CRM category: Pressure Washing
> - Source UUID: olit_0ea934facead4dd9a1ee33e35adba77e
> - Industry UUID: ind_50413bdcbb67410da504845ec48af17d
> - Source file: 50_Reference/CRM Service Descriptions/GilletteWindowSolarCleaning_pricebook_export.csv

> [!warning]- Audit Notes
> - Description mentions pavers even though a separate paver-specific service exists. Clarify the scope so staff and customers do not confuse the offerings.

> [!abstract]- Review History
> - 2026-04-01 - Imported from the CRM pricebook export with `scripts/build_service_catalog.py`.
