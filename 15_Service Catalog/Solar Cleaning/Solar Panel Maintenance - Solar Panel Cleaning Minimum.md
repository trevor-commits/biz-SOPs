---
type: service-description
status: draft
owner: Trevor
created: 2026-04-01
last_reviewed:
next_review:
service_line: Solar Cleaning
service_type: core-service
customer_visibility: customer-facing
review_status: clean
title_collision: false
source_uuid: olit_6cce784706ee4108acb76dec257c666f
price: 200
unit_of_measure:
cost: 0
taxable: false
online_booking_enabled: true
aliases: []
tags:
  - service-description
---

# Solar Panel Cleaning Minimum

> [!summary] Quick Summary
> Dust, pollen, and especially bird droppings can block sunlight and create hot spots that damage panels. We’ll clean your system safely so it keeps producing at its best.

> [!info] Pricing
> - CRM price: $200.00
> - Unit: Quoted / not specified
> - Taxable: No
> - Online booking enabled: Yes
> - Pricing note: Fixed CRM price is present; confirm the pricing basis before using it externally.

## Included
- Rinse panels with purified water
- Gently brush with solar panel–specific tools to remove stuck-on debris
- Light visual check for cracks, shading issues, or anything unusual
- Look underneath for signs of nesting or chewed wiring (where accessible)

## Not Included
- Bird proofing (available separately)
- Repairs to panels, wiring, or roofing materials

## Best Fit / When To Offer
- Improves energy production by letting in more sunlight
- Removes bird droppings that can cause hot spots and etch the glass
- Helps prevent long-term damage and keeps your system performing well

## Just So You Know
We don’t use pressure washers or harsh chemicals — only solar-approved methods. Pricing depends on panel count, roof access, and layout.

## Upsells and Related Services
- [[15_Service Catalog/Solar Cleaning/Solar Panel Maintenance - Debris - Mess Removal|Debris/Mess Removal]]
- [[15_Service Catalog/Solar Cleaning/Solar Panel Maintenance - Solar Panel Bird Proofing|Solar Panel Bird Proofing]]
- [[15_Service Catalog/Solar Cleaning/Solar Panel Maintenance - Solar Panel Cleaning|Solar Panel Cleaning]]

## Internal Notes
- CRM category: Solar Panel Maintenance.
- This row is currently marked as online-bookable in the CRM.
- Use the Local Notes and Links section for approved SOP, checklist, equipment, and purchase-note links.

## Local Notes and Links
- Add approved SOP, checklist, equipment, and purchase-note links here.

> [!note]- Source Copy
> Dust, pollen, and especially bird droppings can block sunlight and create hot spots that damage panels. We’ll clean your system safely so it keeps producing at its best.
>
> What we do:
> • Rinse panels with purified water
> • Gently brush with solar panel–specific tools to remove stuck-on debris
> • Light visual check for cracks, shading issues, or anything unusual
> • Look underneath for signs of nesting or chewed wiring (where accessible)
>
> Why it’s worth it:
> • Improves energy production by letting in more sunlight
> • Removes bird droppings that can cause hot spots and etch the glass
> • Helps prevent long-term damage and keeps your system performing well
>
> What’s not included:
> • Bird proofing (available separately)
> • Repairs to panels, wiring, or roofing materials
>
> Just so you know:
> We don’t use pressure washers or harsh chemicals — only solar-approved methods. Pricing depends on panel count, roof access, and layout.

> [!note]- Source Metadata
> - Industry: Window & Exterior Cleaning
> - CRM category: Solar Panel Maintenance
> - Source UUID: olit_6cce784706ee4108acb76dec257c666f
> - Industry UUID: ind_50413bdcbb67410da504845ec48af17d
> - Source file: 50_Reference/CRM Service Descriptions/GilletteWindowSolarCleaning_pricebook_export.csv

> [!warning]- Audit Notes
> - No auto-detected issues during import.

> [!abstract]- Review History
> - 2026-04-01 - Imported from the CRM pricebook export with `scripts/build_service_catalog.py`.
