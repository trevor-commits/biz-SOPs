---
type: service-description
status: draft
owner: Trevor
created: 2026-04-01
last_reviewed:
next_review:
service_line: Gutters
service_type: repair
customer_visibility: customer-facing
review_status: needs-review
source_uuid: olit_4b8d73e4cbc1408ea1f2e3fcacdf6fc2
price: 0
unit_of_measure: 
cost: 0
taxable: false
online_booking_enabled: true
aliases: []
tags:
  - service-description
---

# Wire Removal from Gutter/Roof

> [!summary] Quick Summary
> Loose leftover wiring can end up sitting in the gutter line or sticking out along the roof edge. We access the roofline and remove the abandoned wire so it’s out of the way.

> [!info] Pricing
> - CRM price: $0.00
> - Unit: Quoted / not specified
> - Taxable: No
> - Online booking enabled: Yes
> - Pricing note: CRM stores this as $0.00. Treat as quoted, complimentary, or incomplete until verified.

## Included
- Set up ladder access to reach the gutter/roof edge area
- Cut and remove abandoned exterior wire (ex: old satellite/low-voltage line)
- Pull wire out of the gutter line and remove loose clips/straps when possible
- Bag/dispose of removed material and clean up

## Not Included
- Electrical work, testing, or reconnecting services
- Tracing where the wire came from or what it powered
- Removing active utility/provider lines
- Repairs to roofing, fascia, or gutter components

## Best Fit / When To Offer
- Keeps wire from catching debris in the gutter line
- Reduces the chance of the wire rubbing or snagging in wind
- Cleans up the roof edge and gutter area

Priced per removal item. We remove what’s accessible and clearly abandoned; anything tied into active equipment is excluded.

## Upsells and Related Services
- [[15_Service Catalog/Gutters/Gutter Repairs - Downspout Elbow Replacement – Ground Level|Downspout Elbow Replacement – Ground Level]]
- [[15_Service Catalog/Gutters/Gutter Repairs - Downspout End Trim & Safety Cap|Downspout End Trim & Safety Cap]]
- [[15_Service Catalog/Gutters/Gutter Repairs - Downspout Outlet Repair – Rivet|Downspout Outlet Repair – Rivet]]

## Internal Notes
- CRM category: Gutter Repairs.
- Pricing is not ready for public use without verification.
- This row is currently marked as online-bookable in the CRM.
- Link related SOPs, checklists, equipment, and purchase notes here as those notes are created.

> [!note]- Source Copy
> Loose leftover wiring can end up sitting in the gutter line or sticking out along the roof edge. We access the roofline and remove the abandoned wire so it’s out of the way.
>
> What we do:
> • Set up ladder access to reach the gutter/roof edge area
> • Cut and remove abandoned exterior wire (ex: old satellite/low-voltage line)
> • Pull wire out of the gutter line and remove loose clips/straps when possible
> • Bag/dispose of removed material and clean up
>
> Why it’s worth it:
> • Keeps wire from catching debris in the gutter line
> • Reduces the chance of the wire rubbing or snagging in wind
> • Cleans up the roof edge and gutter area
>
> What’s not included:
> • Electrical work, testing, or reconnecting services
> • Tracing where the wire came from or what it powered
> • Removing active utility/provider lines
> • Repairs to roofing, fascia, or gutter components
>
> Just so you know:
> Priced per removal item. We remove what’s accessible and clearly abandoned; anything tied into active equipment is excluded.

> [!note]- Source Metadata
> - Industry: Window & Exterior Cleaning
> - CRM category: Gutter Repairs
> - Source UUID: olit_4b8d73e4cbc1408ea1f2e3fcacdf6fc2
> - Industry UUID: ind_50413bdcbb67410da504845ec48af17d
> - Source file: 50_Reference/CRM Service Descriptions/GilletteWindowSolarCleaning_pricebook_template_compatible.csv

> [!warning]- Audit Notes
> - CRM price is $0.00. Treat this as quoted, complimentary, or incomplete until pricing is verified.

> [!abstract]- Review History
> - 2026-04-01 - Imported from the CRM pricebook export with `scripts/build_service_catalog.py`.
