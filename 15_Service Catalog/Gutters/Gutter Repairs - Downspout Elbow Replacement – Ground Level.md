---
type: service-description
status: draft
owner: Trevor
created: 2026-04-01
last_reviewed:
next_review:
service_line: Gutters
service_type: repair
customer_visibility: customer-facing
review_status: clean
title_collision: false
source_uuid: olit_fb96505ef9124aa78b1b1d59f8667cff
price: 60
unit_of_measure: 
cost: 0
taxable: false
online_booking_enabled: true
aliases: []
tags:
  - service-description
---

# Downspout Elbow Replacement – Ground Level

> [!summary] Quick Summary
> A cracked or smashed elbow can dump water where you don’t want it. We replace the elbow, trim for fit, and paint it to blend with the downspout.

> [!info] Pricing
> - CRM price: $60.00
> - Unit: Quoted / not specified
> - Taxable: No
> - Online booking enabled: Yes
> - Pricing note: Fixed CRM price is present; confirm the pricing basis before using it externally.

## Included
- Remove the damaged downspout elbow
- Trim/cut for proper fit as needed
- Install a new plastic elbow (customer-provided or approved equivalent)
- Prep and paint the elbow to match the existing downspout

## Not Included
- Replacing vertical downspout runs or gutter outlets
- Underground drain work or extensions beyond the elbow
- Repainting the full downspout or gutter system
- Repairs to landscaping or concrete from previous drainage issues

## Best Fit / When To Offer
- Directs water away more predictably at the base
- Reduces leaks and splashing at the elbow connection
- Improves appearance with paint to blend

Priced per elbow. Paint is best-match and may look slightly different until it fully cures and weathers.

## Upsells and Related Services
- [[15_Service Catalog/Gutters/Gutter Repairs - Downspout End Trim & Safety Cap|Downspout End Trim & Safety Cap]]
- [[15_Service Catalog/Gutters/Gutter Repairs - Downspout Outlet Repair – Rivet|Downspout Outlet Repair – Rivet]]
- [[15_Service Catalog/Gutters/Gutter Repairs - Downspout Repair|Downspout Repair]]

## Internal Notes
- CRM category: Gutter Repairs.
- This row is currently marked as online-bookable in the CRM.
- Use the Local Notes and Links section for approved SOP, checklist, equipment, and purchase-note links.

## Local Notes and Links
- Add approved SOP, checklist, equipment, and purchase-note links here.

> [!note]- Source Copy
> A cracked or smashed elbow can dump water where you don’t want it. We replace the elbow, trim for fit, and paint it to blend with the downspout.
>
> What we do:
> • Remove the damaged downspout elbow
> • Trim/cut for proper fit as needed
> • Install a new plastic elbow (customer-provided or approved equivalent)
> • Prep and paint the elbow to match the existing downspout
>
> Why it’s worth it:
> • Directs water away more predictably at the base
> • Reduces leaks and splashing at the elbow connection
> • Improves appearance with paint to blend
>
> What’s not included:
> • Replacing vertical downspout runs or gutter outlets
> • Underground drain work or extensions beyond the elbow
> • Repainting the full downspout or gutter system
> • Repairs to landscaping or concrete from previous drainage issues
>
> Just so you know:
> Priced per elbow. Paint is best-match and may look slightly different until it fully cures and weathers.

> [!note]- Source Metadata
> - Industry: Window & Exterior Cleaning
> - CRM category: Gutter Repairs
> - Source UUID: olit_fb96505ef9124aa78b1b1d59f8667cff
> - Industry UUID: ind_50413bdcbb67410da504845ec48af17d
> - Source file: 50_Reference/CRM Service Descriptions/GilletteWindowSolarCleaning_pricebook_template_compatible.csv

> [!warning]- Audit Notes
> - No auto-detected issues during import.

> [!abstract]- Review History
> - 2026-04-01 - Imported from the CRM pricebook export with `scripts/build_service_catalog.py`.
