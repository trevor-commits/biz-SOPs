---
type: service-description
status: draft
owner: Trevor
created: 2026-04-01
last_reviewed:
next_review:
service_line: Gutters
service_type: repair
customer_visibility: customer-facing
review_status: needs-review
title_collision: false
source_uuid: olit_fd304b861862496eb52f23d196a5ce29
price: 0
unit_of_measure:
cost: 0
taxable: false
online_booking_enabled: true
aliases: []
tags:
  - service-description
---

# Downspout Outlet Repair – Rivet

> [!summary] Quick Summary
> If a screw is blocking the downspout outlet, it can catch debris and slow drainage. We remove the obstruction and replace it with a rivet so water can flow through more freely.

> [!info] Pricing
> - CRM price: $0.00
> - Unit: Quoted / not specified
> - Taxable: No
> - Online booking enabled: Yes
> - Pricing note: CRM stores this as $0.00. Treat as quoted, complimentary, or incomplete until verified.

## Included
- Remove the screw or obstruction at the downspout outlet
- Clear the outlet opening at the connection point
- Install a rivet to secure the outlet without blocking flow
- Quick flow check at the outlet area

## Not Included
- Full downspout snaking or clearing beyond the outlet area
- Replacing the outlet, downspout, or gutter section
- Gutter cleaning or roof debris removal
- Leak sealing outside the outlet connection

## Best Fit / When To Offer
- Reduces snag points that can trap leaves and grit
- Helps improve drainage speed at the outlet
- Lowers the chance of repeat clogs at that connection

Priced per outlet. If the outlet or downspout is crushed, split, or heavily corroded, replacement is a separate service item.

## Upsells and Related Services
- [[15_Service Catalog/Gutters/Gutter Repairs - Downspout Elbow Replacement – Ground Level|Downspout Elbow Replacement – Ground Level]]
- [[15_Service Catalog/Gutters/Gutter Repairs - Downspout End Trim & Safety Cap|Downspout End Trim & Safety Cap]]
- [[15_Service Catalog/Gutters/Gutter Repairs - Downspout Repair|Downspout Repair]]

## Internal Notes
- CRM category: Gutter Repairs.
- Pricing is not ready for public use without verification.
- This row is currently marked as online-bookable in the CRM.
- Use the Local Notes and Links section for approved SOP, checklist, equipment, and purchase-note links.

## Local Notes and Links
- Add approved SOP, checklist, equipment, and purchase-note links here.

> [!note]- Source Copy
> If a screw is blocking the downspout outlet, it can catch debris and slow drainage. We remove the obstruction and replace it with a rivet so water can flow through more freely.
>
> What we do:
> • Remove the screw or obstruction at the downspout outlet
> • Clear the outlet opening at the connection point
> • Install a rivet to secure the outlet without blocking flow
> • Quick flow check at the outlet area
>
> Why it’s worth it:
> • Reduces snag points that can trap leaves and grit
> • Helps improve drainage speed at the outlet
> • Lowers the chance of repeat clogs at that connection
>
> What’s not included:
> • Full downspout snaking or clearing beyond the outlet area
> • Replacing the outlet, downspout, or gutter section
> • Gutter cleaning or roof debris removal
> • Leak sealing outside the outlet connection
>
> Just so you know:
> Priced per outlet. If the outlet or downspout is crushed, split, or heavily corroded, replacement is a separate service item.

> [!note]- Source Metadata
> - Industry: Window & Exterior Cleaning
> - CRM category: Gutter Repairs
> - Source UUID: olit_fd304b861862496eb52f23d196a5ce29
> - Industry UUID: ind_50413bdcbb67410da504845ec48af17d
> - Source file: 50_Reference/CRM Service Descriptions/GilletteWindowSolarCleaning_pricebook_template_compatible.csv

> [!warning]- Audit Notes
> - CRM price is $0.00. Treat this as quoted, complimentary, or incomplete until pricing is verified.

> [!abstract]- Review History
> - 2026-04-01 - Imported from the CRM pricebook export with `scripts/build_service_catalog.py`.
